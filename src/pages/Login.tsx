...
import { Mail, Lock, User, ArrowRight, Github, Phone, KeyRound, ShieldCheck } from 'lucide-react';
...
export default function Login() {
  ...
  const { user, isLoading: authLoading, login, register, forgotPassword, resetPassword, completeOAuth } = useAuth();
  const navigate = useNavigate();

  // OAuth providers configured on THIS deployment (public booleans only).
  ...
  useEffect(() => {
    ...
    api.auth.oauthProviders().then(setOauthProviders).catch(() => setOauthProviders({ google: false, github: false }));
  }, []);

  // Start a provider sign-in on the BACKEND (state/PKCE are server-generated;
  // the client secret never touches this bundle). Only a SAFE in-app redirect
  // travels through the flow.
  const startOAuth = (provider: 'google' | 'github') => {
    const params = new URLSearchParams({ redirect: redirectTo });
    const deviceId = (() => { try { return localStorage.getItem('rx-store-device-id') || ''; } catch { return ''; } })();
    if (deviceId) params.set('deviceId', deviceId);
    window.location.assign(`${API_URL}/auth/oauth/${provider}/start?${params.toString()}`);
  };

  // A one-time sign-in code from the web callback page (native shells / TV-style flow).
  const submitSignInCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!signInCode.trim()) return;
    setCodeBusy(true);
    try {
      const res = await completeOAuth(signInCode.trim());
      toast.success(res.status === 'new' ? 'Account created — welcome!' : 'Signed in ✓');
      navigate(safeRedirectTarget(res.redirect) || redirectTo, { replace: true });
    } catch (e: any) {
      toast.error(e?.message || 'That code is invalid or has expired.');
    } finally {
      setCodeBusy(false);
    }
  };

  // Where to land after a successful sign-in: an authenticated page the user
  // was trying to reach (?redirect=…), otherwise Home.
  const redirectTo = safeRedirectTarget(searchParams.get('redirect')) || '/';

  // Already signed in? Don't push the user through the form — straight to the
  // app (or wherever they were heading).
  if (user && !authLoading) return <Navigate to={redirectTo} replace />;
  // Session restoration still in flight: show the shell instead of flashing
  // the form (which would look like the user was signed out).
  if (authLoading) {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center section-container py-12">
        <div className="text-center">
          <img src="/v1.png" alt="RX Store" className="w-16 h-16 rounded-xl object-cover mx-auto mb-4 animate-pulse" />
          <p className="text-rx-gray-medium text-sm">Restoring your session…</p>
        </div>
      </div>
    );
  }
        ...
        <div className="card p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {mode === 'login' && (
              <div className="flex items-center justify-between">
                {/* Persistence is NOT a choice here: RX Store keeps every
                    signed-in device signed in until the user explicitly signs
                    out — so there is no "remember me" checkbox to fake it. */}
                <span className="flex items-center gap-1.5 text-xs text-rx-gray-medium">
                  <ShieldCheck className="w-3.5 h-3.5 text-rx-yellow/80" />
                  RX Store keeps you signed in on this device until you sign out.
                </span>
          ...
          {forgotOpen && (
            <div className="mt-6 p-4 rounded-xl bg-rx-dark-tertiary/50 border border-white/10">
              ...
              {/* Human fallback — send it DIRECTLY to the admin inbox (or email) */}
              <div className="mt-3">
                <p className="text-[11px] leading-relaxed text-rx-gray-medium">
                  No reset email, or no longer have access to that inbox?{' '}
                  <button type="button" onClick={() => setAdminHelp((v) => !v)} className="text-rx-yellow hover:underline font-medium">
                    Send a message directly to the admin
                  </button>{' '}
                  — an admin will sort it for you. (Or{' '}
                  <a className="text-rx-yellow hover:underline font-medium" href={`mailto:support@rxstore.com?subject=${encodeURIComponent('Password reset help — RX Store')}`}>email support</a>.)
                </p>
                {adminHelp && (
                  <div className="mt-3">
                    <ContactAdminForm
                      ...
                      template={'Hi RX Store team,\n\nPlease reset the password for my account.\n\nMy account email: \nMy full name: \n\nThank you.'}
                      fields={[{ key: 'accountEmail', label: 'Account email (the one to reset)', placeholder: 'you@example.com' }]}
                    />
                  </div>
                )}
              </div>
          ...
          {(oauthProviders.google || oauthProviders.github) && (
            <div className={`grid grid-cols-${oauthProviders.google && oauthProviders.github ? '2' : '1'} gap-3`}>
              {oauthProviders.google && (
                <button
                  type="button"
                  onClick={() => startOAuth('google')}
                  className="flex items-center justify-center gap-2 px-4 py-3 bg-rx-dark-tertiary border border-white/10 rounded-xl text-sm text-rx-gray-medium hover:text-white hover:border-white/20 transition-all"
                >
          ...
          <div className="text-center">
            <button
              ...
              className="text-xs text-rx-gray-medium hover:text-white underline underline-offset-2"
            >
              Have a sign-in code from another device?
            </button>
            {codeOpen && (
              <form onSubmit={submitSignInCode} className="mt-3 flex gap-2">
                <input
                  type="text"
                  value={signInCode}
                  onChange={(e) => setSignInCode(e.target.value)}
                  placeholder="One-time sign-in code"
                  className="flex-1 bg-rx-dark border border-white/10 rounded-xl px-3 py-2 text-sm text-white font-mono"
                  autoFocus
                />
                <button type="submit" disabled={codeBusy} className="btn-primary text-sm px-4">
                  {codeBusy ? '…' : 'Sign in'}
                </button>
              </form>
            )}
