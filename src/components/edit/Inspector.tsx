...
import { X, Save, Loader2, Check, Trash2, ArrowUp, ArrowDown, Plus, UploadCloud, RotateCcw, Paintbrush, Monitor, Tablet, Smartphone, History, Link2, Send, Timer, Minus } from 'lucide-react';
...
import { newIntroAd, IntroAd, readAdStats, fetchAdServerStats, AdStat, isAdLive, adWindowLabel, dateForInput, createSponsorShare, listSponsorShares, revokeSponsorShare, sponsorUrl, SponsorShare, INTRO_DURATION_KEY, MIN_INTRO_MS, MAX_INTRO_MS, parseIntroDuration } from '../home/introAds';
...
export default function Inspector() {
  ...
  /** −/+ adjust how long the welcome intro (and each ad) stays on screen — published instantly. */
  const bumpIntroDuration = async (delta: number) => {
    if (durationBusy) return;
    const cur = parseIntroDuration(getEffective(INTRO_DURATION_KEY, ''));
    const next = Math.min(MAX_INTRO_MS, Math.max(MIN_INTRO_MS, cur + delta));
    if (next === cur) return;
    setDurationBusy(true);
    try {
      const ok = await save(INTRO_DURATION_KEY, String(next));
      if (ok) toast.success(`Intro duration → ${(next / 1000).toFixed(1)}s ✓`);
      else toast.error('Could not publish the duration');
    } finally { setDurationBusy(false); }
  };
  const introDurationMs = parseIntroDuration(getEffective(INTRO_DURATION_KEY, ''));
            ...
            {tab === 'content' && (
              <>
                ...
                {d.type === 'introAds' && (
                  <>
                    <p className="text-[11px] text-rx-gray-medium mb-3">
                      One sponsored card shows on the welcome intro per page refresh and every Home click — rotated in this order, auto-dismisses after the duration below, visitors can skip. Refresh your own tab to preview.
                    </p>

                    {/* On-screen time — THE button to adjust how long the intro/ad shows */}
                    <div className="mb-3 rounded-xl border border-rx-yellow/25 p-3 bg-rx-yellow/5">
                      <p className="text-[11px] font-bold text-white flex items-center gap-1.5"><Timer className="w-3.5 h-3.5 text-rx-yellow" /> Intro duration — {(introDurationMs / 1000).toFixed(1)}s</p>
                      <p className="text-[10px] text-rx-gray-medium mt-0.5 mb-2">How long the welcome hero or sponsored card stays on screen before dissolving (1–30s). Publishes instantly, applies to every visitor.</p>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => bumpIntroDuration(-500)}
                          disabled={durationBusy || introDurationMs <= MIN_INTRO_MS}
                          className="flex-1 py-2 rounded-xl bg-rx-dark border border-white/10 text-white text-sm font-bold flex items-center justify-center gap-1 hover:border-rx-yellow/40 disabled:opacity-40 transition-all"
                          title="Half a second shorter"
                        ><Minus className="w-4 h-4" /> 0.5s</button>
                        <span className="text-base font-black text-rx-yellow tabular-nums w-16 text-center">{(introDurationMs / 1000).toFixed(1)}s</span>
                        <button
                          onClick={() => bumpIntroDuration(500)}
                          disabled={durationBusy || introDurationMs >= MAX_INTRO_MS}
                          className="flex-1 py-2 rounded-xl bg-rx-yellow text-rx-dark text-sm font-bold flex items-center justify-center gap-1 hover:bg-rx-yellow-light disabled:opacity-40 transition-all"
                          title="Half a second longer"
                        ><Plus className="w-4 h-4" /> 0.5s</button>
                        {durationBusy && <Loader2 className="w-4 h-4 animate-spin text-rx-gray-medium" />}
                      </div>
                    </div>
                    ...
                    {(list as IntroAd[]).map((ad, i) => {
                      ...
                      return (
                        <div key={ad.id || i} className={`rounded-xl border p-3 mb-3 bg-rx-dark/50 ${ad.enabled ? 'border-rx-yellow/30' : 'border-white/10 opacity-70'}`}>
                          ...
                          {ad.imageUrl && <img src={ad.imageUrl} alt="" className="w-full h-20 object-contain bg-black/40 rounded-lg border border-white/10 mb-2" />}
                          ...
                          <p className="text-[10px] text-rx-gray-medium mb-2">
                            Recommended size: <span className="text-white/80 font-semibold">1200 × 675 px (16:9)</span> — the whole image is always shown in full, never cropped.
                          </p>
                          <div className="flex items-center gap-1.5 mb-2">
